...
def build_report(summary: str, tree: str, git: dict[str, str | None]) -> str:
    ...
    lines.extend(
        [
            ...
            "-" * 80,
            "GET  /                 → { \"Ping\": \"Pong\" }",
            "GET  /pipelines/parse  → accepts pipeline form field (stub returns parsed status)",
            "",
            "Note: Submit button in frontend is styled but not yet wired to backend.",
            "",
            "7. NPM / RUN COMMANDS",
            "-" * 80,
        ]
    ...
    lines.extend(
        [
            ...
            "  ○ Submit → backend integration pending",
            "  ○ Pipeline serialization / execution engine pending",
            "",
            "11. RECOMMENDED NEXT STEPS",
            "-" * 80,
            "  1. Wire SubmitButton to POST pipeline JSON to /pipelines/parse",
            "  2. Serialize nodes + edges + node.data from Zustand on submit",
            "  3. Implement backend pipeline validation and topological sort",
            "  4. Add integration tests for parseTextVariables edge cases",
            "  5. Optional: backend execution stub for LangChain-style chains",
            "",
            "=" * 80,
